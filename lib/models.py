...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, motiond):
        golist = motiond.rpc_command('gobject', 'list')
        ...
        try:
            ...
            for item in golist.values():
                (go, subobj) = self.import_gobject_from_motiond(motiond, item)
    ...
    @classmethod
    def import_gobject_from_motiond(self, motiond, rec):
        import decimal
        import motionlib
        import inflection
        ...
        # shim/motiond conversion
        object_hex = motionlib.SHIM_deserialise_from_motiond(object_hex)
        objects = motionlib.deserialise(object_hex)
        subobj = None
        ...
        # exclude any invalid model data from motiond...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from motiond, with every run
        ...
        # get/create, then sync payment amounts, etc. from motiond - Motiond is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(motiond)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from motiond! %s" % e)
            govobj.vote_delete(motiond)
            return (govobj, None)
    ...
    def vote_delete(self, motiond):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(motiond, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, motiond, signal, outcome):
        ...
        output = motiond.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = motionlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(motiond, signal)

    def sync_network_vote(self, motiond, signal):
        printdbg('\tsyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = motiond.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 motion addr, non-multisig
            if not motionlib.is_valid_motion_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Motion address for network [%s], returning False" % (self.payment_address, config.network))
                return False
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (motionlib.blocks_to_seconds(superblockcycle) / 2) +
    ...
    def is_deletable(self):
        ...
        # TBD (item moved to external storage/MotionDrive, etc.)
        return False
    ...
    def prepare(self, motiond):
        try:
            object_hash = motiond.rpc_command(*self.get_prepare_command())
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not motionlib.is_valid_motion_address(addr, config.network):
    ...
    def hash(self):
        import motionlib
        return motionlib.hashit(self.serialise())
...
class Watchdog(BaseModel, GovernanceClass):
    ...
    @classmethod
    def active(self, motiond):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at >= (now - motiond.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
        return resultset

    @classmethod
    def expired(self, motiond):
        now = int(time.time())
        resultset = self.select().where(
            self.created_at < (now - motiond.SENTINEL_WATCHDOG_MAX_SECONDS)
        )
    ...
    def is_expired(self, motiond):
        now = int(time.time())
        return (self.created_at < (now - motiond.SENTINEL_WATCHDOG_MAX_SECONDS))

    def is_valid(self, motiond):
        if self.is_expired(motiond):
            return False

        return True

    def is_deletable(self, motiond):
        if self.is_expired(motiond):
            return True
