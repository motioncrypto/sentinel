...
class MotionConfig():
    ...
    def slurp_config_file(self, filename):
        # read motion.conf config but skip commented lines
        f = io.open(filename)
        lines = []
        for line in f:
        ...
        # data is motion.conf without commented lines
        data = ''.join(lines)
    ...
    def get_rpc_creds(self, data, network='mainnet'):
        ...
        default_port = 3385 if (network == 'mainnet') else 13385

        # use default port for network if not specified in motion.conf
        if not ('port' in creds):
            creds[u'port'] = default_port

        # convert to an int if taken from motion.conf
